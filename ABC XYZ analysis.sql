...
/* Обычно в XYZ-анализе в качестве временных отрезков берут квартал или полугодие, 
чтобы лучше прослеживалась сезонность, однако, предоставленных мне данных не очень много, 
поэтому беру за ориентир неделю. */
xyz_analysis as(
  SELECT 
   product, 
   CASE 
